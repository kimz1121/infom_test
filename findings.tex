% ============================================================
%  Research Findings  (weekly chapter / to be merged into the proposal)
%
%  USAGE
%   - Standalone: wrap with the same preamble as proposal_single.tex
%     (\documentclass[11pt]{article} ... \begin{document} ... \end{document})
%     and append the bibitems in the marked block at the bottom.
%   - Merge: paste the \section below in before "Proposed Experiments"
%     in proposal_single.tex, and move the new bibitems into its
%     existing thebibliography. Citation keys are namespaced so they
%     will not collide with the proposal's bibliography.
%
%  FRAMING (decided 2026-06-11)
%   - Hybrid: conceptual findings from the literature, then a Preliminary
%     Study reporting our actual inFOM embedding experiments.
%   - The convex-cone / set-membership framing of the older abstract is
%     SET ASIDE. The explicit task embedding is treated method-agnostically
%     as "a separable, learnable, verifiable task representation," of which
%     our intention-conditioned occupancy embedding is one concrete instance
%     (cf. contrastive feature alignment / successor-feature representations).
%   - The abstract/intro/method in proposal_single.tex predate these results;
%     where the direction has moved, THIS chapter is the current direction.
% ============================================================
...
cleaner $10$-task LIBERO-goal benchmark the structure is sharper still
(Fisher~$0.343$; visually distinct $t$-SNE clusters per instruction). An
explicit, separable task representation is therefore \emph{attainable}---the
core feasibility claim behind contribution~(iii) of the proposal.
...
channel). The same lens shows the embedding encodes not just task identity but
intra-task execution variation: the embedding$\leftrightarrow$trajectory
correlation is strong \emph{within} a task ($\rho \approx 0.6$ on state,
$+0.4$ on action) and weak \emph{across} tasks. Two further regularities give us
a usable measurement protocol: the correlation is strongest for short trajectory
windows (it best predicts \emph{near}-future geometry) and strongest
\emph{early} in training---consistent with the Fisher separability peaking around
$50$--$150$k steps and declining thereafter. The headline consequence for the
proposal is methodological: \textbf{we already have a working, label-free way to
verify that a task representation is meaningful}, so Experiment~3 is de-risked.
...
improve.} Representation quality is not uniform across tasks. Constrained,
low-variance motions embed tightly---articulated tasks (open/close/slide,
$\rho \approx 0.53$) and knob tasks (turn on/off, $\rho \approx 0.52$)---while
free-space, high-variance behaviors embed poorly---pick-and-place
($\rho \approx 0.39$) and especially navigation ($\rho \approx 0.25$), with
counter-to-microwave pick-place the worst ($\rho \approx -0.05$). The
interpretation is mechanistic: when motion is tightly constrained, a single
future-summarizing vector predicts the trajectory well; when the task admits
many divergent free-space paths, no single $z$ can. This both validates the
diagnostic (it recovers an intuitive difficulty ordering) and tells us where
representation improvement must be aimed---the compositional, free-space
manipulation regime that long-horizon tasks are made of.
